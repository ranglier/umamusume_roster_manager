...
- l'entree/sortie HTTP de `ReferenceRequestHandler` (`tests/test_http_handler.py`): tests de bout en bout sur un vrai serveur `ThreadingTCPServer` lance sur un port ephemere, couvrant les flux CRUD profils / roster / legacy / builds. Perimetre volontairement limite: jobs d'admin, backups et import/export de profils restent non couverts (fixtures nettement plus lourdes, threads et archives zip)
...
Ce filet couvre maintenant les fonctions pures les plus critiques ainsi que l'orchestration `legacy` et le handler HTTP. Ce qui reste a couvrir en priorite: les jobs d'admin (update/backup asynchrones), l'import/export d'archives de profil, et les routes de sauvegarde (`/api/admin/backups/*`).
