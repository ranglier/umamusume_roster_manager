...
- Lint Python: `pyproject.toml` configure `ruff` (regles `E`/`F`, `E501` ignore volontairement — le style du projet privilegie des lignes longues et plates plutot que le wrapping). Job CI `lint` dans `.github/workflows/tests.yml`, bloquant. Verifie en recuperant le binaire `ruff` directement (pas de `pip` dans cet environnement de dev) — 26 imports inutilises trouves au premier run, tous des restes du refactor `serve_reference.py` (re-export en masse vers `scripts/lib/`). Corriges: les imports genuinement morts ont ete retires, et `tests/test_serve_reference.py` importe maintenant directement depuis `lib.common` / `lib.legacy_factors` / `lib.roster_progression` au lieu de passer par `sr.X` pour les fonctions qui n'etaient re-exportees que pour les tests. `ruff check .` est propre.
