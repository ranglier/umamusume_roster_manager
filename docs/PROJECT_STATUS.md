...
Elle tourne aussi automatiquement sur chaque push / pull request via `.github/workflows/tests.yml`. Le handler HTTP est maintenant couvert de bout en bout sur toutes ses familles de routes (profils, roster, legacy, builds, jobs d'admin, backups, export/import).
